...
export const updateProfile = createAsyncThunk(
  ...
  async (profileData, { rejectWithValue, getState, dispatch }) => {
    try {
      console.log('profileSlice - STARTING UPDATE with data:', profileData);
      ...
      console.log('profileSlice - Sending sanitized data to API:', JSON.stringify(sanitizedData, null, 2));
      ...
      console.log('profileSlice - RAW API response:', response);
      console.log('profileSlice - Profile API response data:', JSON.stringify(response.data, null, 2));
      
      // Detailed logging of specific fields we're having trouble with
      if (response.data && response.data.user) {
        console.log('profileSlice - Phone in response:', response.data.user.phone);
        console.log('profileSlice - Address in response:', response.data.user.address);
      } else if (response.data) {
        console.log('profileSlice - Phone in response:', response.data.phone);
        console.log('profileSlice - Address in response:', response.data.address);
      }
      ...
      try {
        ...
        if (userData) {
          ...
          console.log('profileSlice - Saved user data to localStorage. Phone:', userData.phone, 'Address:', userData.address);
        } else {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateProfile.fulfilled, (state, action) => {
        ...
        console.log('profileSlice - REDUCER: Update fulfilled with payload:', action.payload);
        
        if (action.payload && action.payload.user) {
          console.log('profileSlice - REDUCER: Using server response directly');
          ...
          console.log('profileSlice - REDUCER: Using direct payload response');
          // Replace the user object entirely with the server response
          state.user = action.payload;
        }
        
        console.log('profileSlice - REDUCER: Final user state:', JSON.stringify(state.user, null, 2));
        state.error = null;
      })
